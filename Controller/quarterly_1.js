...
function generateEMILoanSchedule({
  tranche_amount,
  tenure_months,
  interest_rate,
  tranche_date,
  ...
  const quarterlyRepaymentAmount = Math.round(tranche_amount / Math.floor(tenure_months / 3));
  let remainingPrincipal = tranche_amount;
  ...
  for (let i = 0; i < tenure_months; i++) {
    ...
    const interest = Math.round(remainingPrincipal * effectiveRate);

    let principalRepayment = 0;

    // Principal is repaid quarterly (i.e., every 3rd month)
    if ((i + 1) % 3 === 0) {
      principalRepayment = Math.min(quarterlyRepaymentAmount, remainingPrincipal);
      remainingPrincipal -= principalRepayment;
    }
    // Fixing the openingBalance to reflect the previous month correctly
    const openingBalance = Math.round(remainingPrincipal + principalRepayment);
...
exports.generateEMILoanScheduleFile = async (req, res) => {
  ...
  flag
    ...
    const { schedule } = generateEMILoanSchedule({
      tranche_amount,
      tenure_months,
      interest_rate,
      tranche_date,
...
const repaymentRows = schedule.map(row => ({
  ...
  closing_balance: parseInt(row.closingBalance),
...
await repayment_schedule_temp.bulkCreate(repaymentRows);
    ...
    if (input_file_format === '.csv') {
      ...
      res.setHeader('Content-Disposition', 'attachment; filename=EMILoanSchedule.csv');
      ...
      const sheet = workbook.addWorksheet('Loan Schedule - EMI');
      ...
      res.setHeader('Content-Disposition', 'attachment; filename=EMILoanSchedule.xlsx');
